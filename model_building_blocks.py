...
def sinusoidal_embedding(x: tf.Tensor, noise_embedding_size: int) -> tf.Tensor:
    frequencies = tf.exp(
        tf.linspace(
            tf.math.log(1.0),
            tf.math.log(1000.0),
...
class ResidualBlock(keras.layers.Layer):
    def __init__(self, width: int, **kwargs: Any) -> None:
        ...
        self.batch_norm_layer = keras.layers.BatchNormalization(center=False, scale=False)
        self.conv_layer_1 = keras.layers.Conv2D(width, **kwargs, activation=keras.activations.swish)
...
class UpBlock(keras.layers.Layer):
    ...
    def call(self, input_list: list[tf.Tensor, list[tf.Tensor]]) -> tf.Tensor:
        x, skips = input_list
